$(function(){
   ...
   // $("body").append($("<h1 class='text-center'>参会证</h1><hr/>")).append(_buildinfo());
    ...
    $("body").append($("<h1 class='text-center'>参会证</h1><hr/>")).append(_buildTheme(data)).append(_buildinfo()).append($("<hr/><strong>日程安排</strong><hr/>")).append(scheduleDiv).append(popDiv);
    _buildSchedule();
    ...
    function _buildTheme(data){
        ...
        var subject=$('<span>'+data.theme["subject"]+'</span>');
        $(div).append(logo);
        $(div).append(subject);
        $(div).append('<hr/>');
        return div;
   ...
   function _buildSchedule() {
       ...
       for (var i = 0; i < len; i++) {
           ...
           var str='<li><a href="javascript:void(0)" data-index="'+index+'"><span data-index="'+index+'" name="time">'+arr[i]["duration"]+'</span><span data-index="'+index+'"  name="event">'+arr[i]["event"]+'</span></a></li>';
           index++;
